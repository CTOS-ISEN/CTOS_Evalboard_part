...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomSsHdle;                    /**< start_survey handle */
  uint16_t  CustomCvHdle;                  /**< check_validation handle */
  uint16_t  CustomRvHdle;                  /**< retrieve_validation handle */
...
#define COPY_START_SURVEY_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0xfe,0xed,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_CHECK_VALIDATION_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xab,0xcd,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_RETRIEVE_VALIDATION_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x12,0x34,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomCvHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            ...
            Notification.Custom_Evt_Opcode = CUSTOM_STM_CV_WRITE_NO_RESP_EVT;
...
void SVCCTL_InitCustomSvc(void)
{
  ...
  COPY_START_SURVEY_UUID(uuid.Char_UUID_128);
   ...
   *  check_validation
   */
  COPY_CHECK_VALIDATION_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomSsHdle,
                          ...
                          &(CustomContext.CustomCvHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : CV, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : CV \n\r");
  }
   ...
   *  retrieve_validation
   */
  COPY_RETRIEVE_VALIDATION_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomSsHdle,
                          UUID_TYPE_128, &uuid,
                          SizeRv,
                          CHAR_PROP_READ,
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : RV, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : RV \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CV:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value CV command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value CV command\n\r");
      }
      /* USER CODE BEGIN CUSTOM_STM_App_Update_Service_1_Char_1*/

      if(pPayload[0] == 0xAB) {
    	  startACK_flag = startACK_flag == 0 ? 1 : 0;
    ...
    case CUSTOM_STM_RV:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value RV command, result : 0x%x \n\r", ret);
      }
      else
      {
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CV:
	  ...
	  Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomSsHdle, CustomContext.CustomCvHdle, SizeCv, pPayload);
    ...
    case CUSTOM_STM_RV:
	  ...
	  Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomSsHdle, CustomContext.CustomRvHdle, SizeRv, pPayload);
